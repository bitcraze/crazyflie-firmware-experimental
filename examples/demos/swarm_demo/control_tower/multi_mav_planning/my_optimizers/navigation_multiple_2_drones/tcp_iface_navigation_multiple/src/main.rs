///
/// Auto-generated TCP server for optimizer: navigation_multiple
/// Generated at: 2022-06-10 15:31:28.488283
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != NAVIGATION_MULTIPLE_NUM_DECISION_VARIABLES {
    ...
    if let Some(y0) = &execution_parameter.initial_lagrange_multipliers {
        if y0.len() != NAVIGATION_MULTIPLE_N1 {
    ...
    if parameter.len() != NAVIGATION_MULTIPLE_NUM_PARAMETERS {
...
fn run_server(tcp_config: &TcpServerConfiguration) {
    ...
    let mut p = [0.0; NAVIGATION_MULTIPLE_NUM_PARAMETERS];
    let mut cache = initialize_solver();
    info!("Done");
    let listener = TcpListener::bind(format!("{}:{}", tcp_config.ip, tcp_config.port)).unwrap();
    let mut u = [0.0; NAVIGATION_MULTIPLE_NUM_DECISION_VARIABLES];
...
fn main() {
    let matches = App::new("OpEn TCP Server [navigation_multiple]")
        .version("0.0.0")
        .author("John Smith")
