...
class TrajectoryDebugger:
    """
    Class that plots the trajectory and the waypoints used to generate it.
    ...
    def debug_traj_generation(waypoints:np.ndarray, tr:Trajectory,waypoints_used_for_gen,plt_title=None):
        ...
        USE_ONE_FIGURE=1
        if USE_ONE_FIGURE:
            TrajectoryDebugger.grid_spec_plot(waypoints_original, x, y, z, t_traj, x_wps, y_wps, z_wps, t_wps,t_wps_original, fig)
        else:
            TrajectoryDebugger.separate_plots(waypoints_original, x, y, z, t_traj, x_wps, y_wps, z_wps, t_wps, t_wps_original, fig)
